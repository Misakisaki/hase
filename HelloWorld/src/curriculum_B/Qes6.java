	...
	public static void main(String[] args) {
		...
		for (String check : checks) {
			...
			case "パソコン":
				...
				System.out.println("パソコンの残り台数は" + rand.nextInt(11) + 1 + "台です");
				break;
			// 製品名が冷蔵庫が入力された場合
			case "冷蔵庫":
				// ランダムクラスを使用し、冷蔵庫の残り台数をランダムな値でコンソールにて出力
				System.out.println("冷蔵庫の残り台数は" + rand.nextInt(11) + 1 + "台です");
				break;

			// 製品名が扇風機が入力された場合
			case "扇風機":
				// ランダムクラスを使用し、扇風機の残り台数をランダムな値でコンソールにて出力
				System.out.println("扇風機の残り台数は" + rand.nextInt(11) + 1 + "台です");
				break;

			// 製品名が洗濯機が入力された場合
			case "洗濯機":
				// ランダムクラスを使用し、洗濯機の残り台数をランダムな値でコンソールにて出力
				System.out.println("洗濯機の残り台数は" + rand.nextInt(11) + 1 + "台です");
				break;

			// 製品名が加湿器が入力された場合
			case "加湿器":
				// ランダムクラスを使用し、加湿器の残り台数をランダムな値でコンソールにて出力
				System.out.println("加湿器の残り台数は" + rand.nextInt(11) + 1 + "台です");
				break;
			...
			case "ディスプレイ":
				// テレビの残り台数をランダムな台数に代入
				int tv_rand = rand.nextInt(11) + 1;
				// 条件演算子を使いテレビとディプレイのうちテレビですか？という条件
				int result = check.equals("テレビ") ? tv_rand : (11) + 1 - tv_rand;
